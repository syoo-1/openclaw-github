...
### PG 外来对象安装硬闸
任何新插件 / 外来工具 / MCP / app / 脚本 / Skill，进入 PG 正式运行体前，必须先过安检器。

未通过安检或未获老林明确同意：
- 不得装入 PG
- 不得接入 PG 正式运行体
- 不得写入 02-brain
- 只能留在 `~/安检区/待检区` 或 `~/安检区/外挂区`
...
当用户明确说“安检器”，或检查对象是外来工具 / GitHub 项目 / app / 插件 / MCP / 脚本 / 安装命令 / 外来 Skill 时：
- 优先读取 `brain/PG-GUARD-SKILLS/ACTIVE/004-安检器-SKILL-v0.1.md`
- 不得因为出现“检查 / 放行 / 是否安全 / 能不能装”等动作词，误转入 002 全身扫描
- 不得把外来对象安检任务改写成 PG 当前体达标扫描

`brain/PG-GUARD-SKILLS/ACTIVE/002-PG-03-达标扫描-SKILL-v0.1.md` 只用于：
- PG 当前体达标检查
- 03 标准扫描
- PG 系统失控
- 前层降权异常
...
近一，按 005 给新 Core 做一次体检

应优先读取：

brain/PG-GUARD-SKILLS/ACTIVE/005-PG-新Core常规体检-SKILL-v0.1.md

该任务属于新 Core 监管体检，不属于 002-PG-03-达标扫描，也不属于 004-安检器。
